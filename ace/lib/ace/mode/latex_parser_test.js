...
module.exports = {
    ...
    "test: one discussion": function() {
        ...
        assert.equal(JSON.stringify(discussion.textLines), JSON.stringify(['foo comment']));
    ...
    "test: one discussion with reply": function() {
        ...
        assert.equal(JSON.stringify(reply.textLines), JSON.stringify(['bar reply']));
    },

    "test: one discussion with reply splitted by empty lines": function() {
        var doc = new Document([
            "context line",
            "% * <foo@gmail.com> dt1:",
            "% foo comment",
    ...
    "test: one discussion with multiple replies": function() {
        ...
        assert.equal(JSON.stringify(reply.textLines), JSON.stringify(['bar3 reply']));
    },
    ...
    "test: multiple discussions": function() {
        ...
        assert.equal(JSON.stringify(discussion1.textLines), JSON.stringify(['foo comment']));

        var discussion2 = discussions[1];
        assert.equal(discussion2.author, 'bar@gmail.com');
        assert.equal(JSON.stringify(discussion2.textLines), JSON.stringify(['bar comment']));
    },
    ...
    "test: correct discussion heading format": function() {
        ...
        assert.equal(discussions.length, 1);
        var discussion1 = discussions[0];
        assert.equal(discussion1.author, 'Empty Datetime');
        assert.equal(JSON.stringify(discussion1.textLines), JSON.stringify(['empty datetime']));
    },
