...
for i=2:1:N-1
    for j=2:1:N-1
        new_u_arr(i, j) = u_arr(i, j) + w * ( new_u_arr(i-1, j) + u_arr(i+1, j) + new_u_arr(i, j-1) + u_arr(i, j+1) + h_x^2 * f(x(i), y(j)) )/4; 
    end
end
...
while((norma(new_u_arr - u_tochnoe) / start_norma) > eps) 
    ...
    for i=2:1:N-1
        for j=2:1:N-1
            new_u_arr(i, j) = u_arr(i, j) + w * (-4 * u_arr(i, j) + new_u_arr(i-1, j) + u_arr(i+1, j) + new_u_arr(i, j-1) + u_arr(i, j+1) + h_x^2 * f(x(i), y(j))) / 4;
        end
    end
