...
namespace InverseInterpolation
{
	class InverseInterpolation
	{
		static void Main(string[] args)
		{
			...
			{
				t0 = t1;
				t1 = Inverseinterpolation(f, t0, y);
			}
			while(Math.Abs(t1 - t0) > 0.00000001);
			Console.WriteLine();
			Console.WriteLine();
			Console.WriteLine("y* = {0}", y);
			Console.WriteLine("x* = {0}", 0.2 + t1 * 0.1);
			Console.WriteLine("Контроль производной пройден!!");
			Console.ReadLine();
		...
		static double Inverseinterpolation(double[] f, double t, double g)
		{
			double[] y = new double[4];
			y[0] = f[2+1] - f[1+1]; 
			y[1] = f[3+1] - 2 * f[2+1] + f[1+1];
			y[2] = f[4+1] - 3 * f[3+1] + 3 * f[2+1] - f[1+1];
			y[3] = f[5+1] - 4 * f[4+1] + 6 *f[3+1] - 4 * f[2+1] + f[1+1];
			return
				((g - f[2] - 
				t * (t - 1) * y[1] / 2 - 
				t * (t - 1) * (t - 2) * y[2] / 6 - 
				t * (t - 1) * (t - 2) * (t - 3) * y[3] / 24) / y[0]);
		}
